...
Ray PerspectiveCamera::generateRay(const Vec2f & point)
{
	...
	float hScale = point.x() - 0.5f;
	float uScale = point.y() - 0.5f;

	float hAngle = (float)(angle) * hScale;
	float uAngle = (float)(angle) * uScale;

	Vec3f dir = direction;
	Matrix firstTrans = Matrix::MakeYRotation(-uAngle);
	firstTrans.Transform(dir);
	dir.Normalize();

	Vec3f rAxis;
	Vec3f::Cross3(rAxis, dir, up);
	Matrix secTrans = Matrix::MakeAxisRotation(rAxis, hAngle);
	secTrans.Transform(dir);
...
void PerspectiveCamera::initialize()
{
	...
	if (temp > 1.0e-6 || temp < (-1) * 1.0e-6)                        // if orthonal to direction vector ~~~
	{
